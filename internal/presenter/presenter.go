...
func NewAppendOnly(out io.Writer, color bool) RunPresenter {
	return &appendOnlyPresenter{out: out, color: color}
}

func NewLive(out io.Writer, color bool) RunPresenter {
	return &livePresenter{out: out, color: color}
}

type appendOnlyPresenter struct {
	out   io.Writer
	color bool
}

func (p *appendOnlyPresenter) Report(event progress.Event) {
	if p.out == nil {
		return
	}
...
func (p *appendOnlyPresenter) Render(result core.RunResult) {
	if p.out == nil {
		return
	}
